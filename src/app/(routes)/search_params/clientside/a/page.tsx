...
const SearchParamsPageAClientSide = ({ searchParams }: { searchParams: Promise<{ name: string }> }) => { //Can't use async/await in a client component.

    const { name }: { name: string} = use(searchParams);
...
export default requireAuthCompositionPattern(SearchParamsPageAClientSide, account_required);
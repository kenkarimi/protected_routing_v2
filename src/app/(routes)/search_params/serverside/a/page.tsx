...
const SearchParamsPageAServerSide = async ({ searchParams }: { searchParams: Promise<{ name: string }> }) => { //Async/await can only be used in server components.

    const { name }: { name: string} = await searchParams;
...
export default requireAuthCompositionPattern(SearchParamsPageAServerSide, account_required);